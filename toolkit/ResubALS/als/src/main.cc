...
parser CommPars(int argc, char * argv[]) {
    ...
    option.add<string>("lacType", '\0', "Resubstitution LAC Type: CONST, SASIMI, APPRESUB", false, "APPRESUB");
...
void ConfigureOptions(ALSOpt & alsOpt, string & accCirc, string & metrType, string & distrType, string & lacType) {
    // load standard cell
    AbcMan abcMan;
    ...
    // configure LAC type
    if (lacType == "CONST")
        alsOpt.lacType = LAC_TYPE::CONS;
    else if (lacType == "SASIMI")
        alsOpt.lacType = LAC_TYPE::SASIMI;
    else if (lacType == "RAC")
...
int app_resub_main(int argc, char * argv[]) {
    ...
    // configure options
    ConfigureOptions(alsOpt, accCirc, metrType, distrType, lacType);

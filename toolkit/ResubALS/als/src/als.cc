...
NetManPtr ALSMan::BuildXorOrCircuit(int nBits) {
    ...
    for (int i = 0; i < nBits; ++i) {
        ...
        piA[i] = ABC_NAMESPACE::Abc_NtkCreatePi(pAbcNet);
    }
    for (int i = 0; i < nBits; ++i) {
        // cout << "create PIB " << i << endl;
        piB[i] = ABC_NAMESPACE::Abc_NtkCreatePi(pAbcNet);
    }
    ...
    // create POs
    // for (int i = 0; i < nBits; ++i) {
    //     auto pPo = ABC_NAMESPACE::Abc_NtkCreatePo(pAbcNet);
    //     ABC_NAMESPACE::Abc_ObjAddFanin(pPo, diff[i]);
    // }
    auto pPo = ABC_NAMESPACE::Abc_NtkCreatePo(pAbcNet);
    ABC_NAMESPACE::Abc_ObjAddFanin(pPo, pOr);
...
NetManPtr ALSMan::BuildAbsoluteDifferenceCircuit(int nBits) {
    ...
    auto pConst0 = ABC_NAMESPACE::Abc_NtkCreateNodeConst0(pAbcNet);
    auto pConst1 = ABC_NAMESPACE::Abc_NtkCreateNodeConst1(pAbcNet);
    ...
    for (int i = 0; i <= nBits; ++i)
        negB[i] = ABC_NAMESPACE::Abc_NtkCreateNodeInv(pAbcNet, extB[i]);
    ...
    carry2[0] = pConst1;
    for (int i = 0; i < nBits; ++i) {
        auto pNegSum = ABC_NAMESPACE::Abc_NtkCreateNodeInv(pAbcNet, sum[i]);
    ...
    for (int i = 0; i < nBits; ++i)
        diff[i] = ABC_NAMESPACE::Abc_NtkCreateNodeMux(pAbcNet, sum[nBits], comp[i], sum[i]);
...
Int2DVect TempApplyLacs(NetMan& net, LACPtrVect& lacs, LAC_TYPE lacType, bool isVerb) {
    ...
    for (const auto & pLac: lacs) {
        ABC_NAMESPACE::Abc_Obj_t* pTS = nullptr, *pSS = nullptr;

...
vector <int> NetMan::GetTFI(ABC_NAMESPACE::Abc_Obj_t * pObj, const set <int> & critGraph) const {
    vector <int> objs;
...
void NetMan::GetTFIRec(ABC_NAMESPACE::Abc_Obj_t * pObj, std::vector <int> & objs, const set <int> & critGraph) const {
...
vector <int> NetMan::GetTFO(ABC_NAMESPACE::Abc_Obj_t * pObj, const set <int> & critGraph) const {
    vector <int> objs;
    objs.reserve(GetNodeNum());
    SetNetNotTrav();
    for (int i = 0; i < GetFanoutNum(pObj); ++i) {
...
void NetMan::GetTFORec(ABC_NAMESPACE::Abc_Obj_t * pObj, std::vector <int> & objs, const set <int> & critGraph) const {
    if (critGraph.count(pObj->Id) == 0)
        return;
    // if (!IsNode(pObj))
    //     return;
...
void NetMan::SATSweep() {
    ...
    AbcMan::Comm("ps; st; ps; ifraig; ps;");
    pNtk = Abc_NtkDup(AbcMan::GetNet()); // NetMan manage the memory of the duplicated network
}
...
IntVec NetMan::TempRepl(ABC_NAMESPACE::Abc_Obj_t * pTS, ABC_NAMESPACE::Abc_Obj_t * pSS) {
    #ifdef DEBUG
    assert(pTS != pSS);
...
ABC_NAMESPACE::Abc_Obj_t* NetMan::CreateNode(const std::vector<ABC_NAMESPACE::Abc_Obj_t*>& pFanins, const std::string& sop) {

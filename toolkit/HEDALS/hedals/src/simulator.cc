...
// Simulator::Simulator(Abc_Ntk_t * p_ntk, unsigned _seed): NetMan(p_ntk, false), seed(_seed) {
//     #ifdef DEBUG
//     auto type = NetMan::GetNetType();
//     assert(type == NET_TYPE::AIG || type == NET_TYPE::GATE || type == NET_TYPE::SOP);
//     #endif
//     if (!NetMan::IsInTopoOrd())
//         NetMan::TopoSort();
//     #ifdef DEBUG
//     assert(NetMan::IsInTopoOrd());
//     #endif
//     dat.resize(NetMan::GetIdMaxPlus1());
// }


// Simulator::Simulator(string & fileName, unsigned _seed): NetMan(fileName), seed(_seed) {
// }
...
double Simulator::GetMeanHammDist(const Simulator & oth_smlt, bool isCheck) const {
    ...
    ll sumHammDist = 0;
    for (int i = 0; i < NetMan::GetPoNum(); ++i) {
        auto diff = (this->dat[NetMan::GetPoId(i)] ^ oth_smlt.dat[oth_smlt.GetPoId(i)]);

...
public:
    explicit LAC(): ER(LL_MAX), MEM(LL_MAX), targId(-1) {}
    explicit LAC(int targ_node_id): ER(LL_MAX), MEM(LL_MAX), targId(targ_node_id) {}
    virtual ~LAC() = default;
    ...
    inline BigInt CalcMEM(NetMan& net, double T);
    inline BigInt GetER() const {return ER;}
    inline BigInt GetMEM() const {return MEM;}
    inline int GetMaxPOId() const {return maxPOId;}
    inline void SetER(BigInt& err_big_int) {ER = err_big_int;}
    inline void SetER(int err) {ER = (BigInt)(err);}
    inline void SetMEM(BigInt& err_big_int) {MEM = err_big_int;}
    inline void SetMEM(int err) {MEM = (BigInt)(err);}
    inline int GetTargId() const {return targId;}
    inline void SetTargId(ll targ_node_id) {targId = targ_node_id;}
    inline void Print(bool isNewLine) const {std::cout << "node " << targId << ", " << "ER = " << ER << ", " << "MEM = " << MEM; if (isNewLine) std::cout << std::endl;}
};
...
public:
    ...
    std::vector<LACPtr> GetCandLACswithERMEM(NetMan& net, double uppER, double uppMEM, double T, int nFrame) const;
    ...
    void GetDivs(ABC_NAMESPACE::Abc_Obj_t* pNode, int nLevDivMax, RETURN_VAR IntVect& divs);

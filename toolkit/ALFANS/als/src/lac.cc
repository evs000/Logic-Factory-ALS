...
BigInt LAC::CalcMEM(NetMan& net, double T) {
    ABC_NAMESPACE::Abc_Obj_t* targetNode = net.GetObj(targId);
    
    std::vector<int> reachablePOs = net.GetReachablePOIds(targetNode);
    ...
    int minPOId = *std::min_element(reachablePOs.begin(), reachablePOs.end());
    maxPOId = *std::max_element(reachablePOs.begin(), reachablePOs.end());
    // Calc MEM using: Smem = e^(imin/T)
    MEM = static_cast<BigInt>(std::exp(static_cast<double>(minPOId) / T));

    return MEM;
}

// int LAC::CalcMaxPOMag(NetMan& net) {
//     ABC_NAMESPACE::Abc_Obj_t* targetNode = net.GetObj(targId);
...
//     int maxPOId = *std::max_element(reachablePOs.begin(), reachablePOs.end());
...
void LACMan::Gen012ResubLACsPro(NetMan& net, IntVect& nodeIds, unsigned seed, int maxLevelDiff, int nFrame4ResubGen, int maxCandResub) {
    ...
    for (int targId: targIds) {
        ...
        GetDivs(pTarget, ABC_NAMESPACE::Abc_ObjRequiredLevel(pTarget) - 1, divs4Nodes[targId]);
        ++pd;
    }
    ...
    for (int targId: targIds) {
        ...
        for (int div: divs) {
            if (div == fanin0 || div == fanin1)
                continue;
            int levelDiff = ABC_NAMESPACE::Abc_ObjReverseLevel(net.GetObj(div)) - ABC_NAMESPACE::Abc_ObjReverseLevel(net.GetObj(targId));
    ...
    // clean up
    ABC_NAMESPACE::Abc_NtkStopReverseLevels(net.GetNet());
}
...
std::vector<LACPtr> LACMan::GetCandLACswithERMEM(NetMan& net, double uppER, double uppMEM, double T, int nFrame) const {
    assert(pLacs.size());
    std::vector<LACPtr> pCandLACs;
    double maxScore = -1.0;

    for (ll i = 0; i < pLacs.size(); ++i) {
        auto pLac = pLacs[i];
        auto pResubLac = dynamic_pointer_cast<ResubLAC>(pLac);
        double curER = double(BigFlt(pLac->GetER()) / BigFlt(nFrame));
        if (curER <= uppER) {
            int curSizeGain = pResubLac->GetSizeGain();
            double curAppMEM = static_cast<double>(pLac->CalcMEM(net, T));
            double curScore = static_cast<double>(curSizeGain)/(curER * curAppMEM);
            if (curScore >= maxScore) {  
                // cout<<"new max score "<<i<<": (sizeGain)"<<curSizeGain<<" (ER)"<<curER<<" (MEM)"<<curAppMEM<<" (curScore)"<<curScore<<endl;
                if (curAppMEM <= uppMEM){
                    pCandLACs.push_back(pLac);
                    maxScore = curScore;
                }
            }
        }
    }
    cout<<"get "<<pCandLACs.size()<<" candidate LACs"<<endl;
    return pCandLACs;
}

...
void VECBEEMan::BatchErrEstPro(NetMan& accNet, NetMan& appNet, LACMan& lacMan, bool enableFastErrEst) {
...
void VECBEEMan::ExpandCut(Abc_Obj_t* pObj, AbcObjList& disjCut) {
    ABC_NAMESPACE::Abc_Obj_t * pFanout = nullptr;
    ll i = 0;
    Abc_ObjForEachFanout(pObj, pFanout, i) {
        if (!ABC_NAMESPACE::Abc_NodeIsTravIdCurrent(pFanout)) {
            if (ABC_NAMESPACE::Abc_ObjFanoutNum(pFanout) || ABC_NAMESPACE::Abc_ObjIsPo(pFanout)) {
                ABC_NAMESPACE::Abc_NodeSetTravIdCurrent(pFanout);
...
Abc_Obj_t* VECBEEMan::ExpandWhich(AbcObjList& disjCut) {
    for (auto ppAbcObj1 = disjCut.begin(); ppAbcObj1 != disjCut.end(); ++ppAbcObj1) {
        ...
        for (++ppAbcObj2; ppAbcObj2 != disjCut.end(); ++ppAbcObj2) {
            ...
            if (isJoint.any()) {
                ABC_NAMESPACE::Abc_Obj_t * pRet = nullptr;
    ...
    for (int index = start; index < end; ++index) {
        ...
        for (auto& pInner: cutNtk) {
            ...
            if (Abc_ObjIsPo(pInner)) {
                assert(!ABC_NAMESPACE::Abc_ObjIsComplement(pInner));
                Abc_Obj_t* pDriver = ABC_NAMESPACE::Abc_ObjFanin0(pInner);
...
static void CalcSomeBoolDiffPo2Node(Simulator & appSmlt, std::vector < std::vector < boost::dynamic_bitset <ull> > > & bdPo2Nodes, vector <Abc_Obj_t *> & topoNodes, std::vector < std::list <ABC_NAMESPACE::Abc_Obj_t *> > & disjCuts, std::vector < std::vector < boost::dynamic_bitset <ull> > > & bdCut2Nodes, timer::progress_display & pd, ll start, ll end) {

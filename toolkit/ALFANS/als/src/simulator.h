...
public:
    ...
    void CalcLocBoolDiff(ABC_NAMESPACE::Abc_Obj_t * pObj, std::list <ABC_NAMESPACE::Abc_Obj_t *> & disjCut, std::vector <ABC_NAMESPACE::Abc_Obj_t *> & cutNtk, std::vector < BitVect > & bdCut2Node);
    void CalcLocPartDiff(ABC_NAMESPACE::Abc_Obj_t * pObj, std::list <ABC_NAMESPACE::Abc_Obj_t *> & disjCut, std::vector <ABC_NAMESPACE::Abc_Obj_t *> & cutNtk, std::vector < std::vector <int8_t> > & pdCut2Node);
    void UpdSopNodeForBoolAndPartDiff(ABC_NAMESPACE::Abc_Obj_t * pObj);
    void UpdGateNodeForBoolAndPartDiff(ABC_NAMESPACE::Abc_Obj_t * pObj);
    void UpdSopForBoolAndPartDiff(ABC_NAMESPACE::Abc_Obj_t * pObj, char * pSop);

    inline ll GetFrameNumb() {return nFrame;}
    inline BitVect * GetDat(ABC_NAMESPACE::Abc_Obj_t * pObj) {return &dat[GetId(pObj)];}
    inline BitVect * GetDat(ll id) {return &dat[id];}
    inline bool GetDat(ABC_NAMESPACE::Abc_Obj_t * pObj, ll frameId) {return dat[GetId(pObj)][frameId];}
    inline bool GetDat(ll id, ll frameId) {return dat[id][frameId];}
    inline BitVect * GetTempDat(ABC_NAMESPACE::Abc_Obj_t * pObj) {return &tempDat[GetId(pObj)];}

...
bool AbcMan::CheckSCLNet(ABC_NAMESPACE::Abc_Ntk_t * pNtk) const {
    Abc_Obj_t * pObj, * pFanin;
    int i, k, fFlag = 1;
...
vector <Abc_Obj_t *> NetMan::GetTFI(ABC_NAMESPACE::Abc_Obj_t * pObj) const {
    vector <Abc_Obj_t *> nodes;
    nodes.reserve(GetNodeNum());
    SetNetNotTrav();
    for (ll i = 0; i < GetFaninNum(pObj); ++i) {
...
void NetMan::GetTFIRec(ABC_NAMESPACE::Abc_Obj_t * pObj, std::vector <ABC_NAMESPACE::Abc_Obj_t *> & nodes) const {
    if (!IsNode(pObj))
        return;
    SetObjTrav(pObj);
    for (ll i = 0; i < GetFaninNum(pObj); ++i) {
...
vector <ll> NetMan::GetTFI(ABC_NAMESPACE::Abc_Obj_t * pObj, const set <ll> & critGraph) const {
    vector <ll> objs;
...
void NetMan::GetTFIRec(ABC_NAMESPACE::Abc_Obj_t * pObj, std::vector <ll> & objs, const set <ll> & critGraph) const {
...
vector <Abc_Obj_t *> NetMan::GetTFO(ABC_NAMESPACE::Abc_Obj_t * pObj) const {
    vector <Abc_Obj_t *> nodes;
    nodes.reserve(GetNodeNum());
    SetNetNotTrav();
...
void NetMan::GetTFORec(ABC_NAMESPACE::Abc_Obj_t * pObj, std::vector <ABC_NAMESPACE::Abc_Obj_t *> & nodes) const {
    if (!IsNode(pObj))
        return;
    SetObjTrav(pObj);
    for (ll i = 0; i < GetFanoutNum(pObj); ++i) {
...
vector <ll> NetMan::GetTFO(ABC_NAMESPACE::Abc_Obj_t * pObj, const set <ll> & critGraph) const {
    vector <ll> objs;
    objs.reserve(GetNodeNum());
    SetNetNotTrav();
    for (ll i = 0; i < GetFanoutNum(pObj); ++i) {
...
void NetMan::GetTFORec(ABC_NAMESPACE::Abc_Obj_t * pObj, std::vector <ll> & objs, const set <ll> & critGraph) const {
    if (critGraph.count(pObj->Id) == 0)
        return;
    // if (!IsNode(pObj))
    //     return;
...
std::vector <int> NetMan::GetReachablePOIds(ABC_NAMESPACE::Abc_Obj_t * pObj) const {
    vector <int> reachablePOs;

    vector <Abc_Obj_t *> TFONodes = GetTFO(pObj);
    for (auto node : TFONodes) {
        if (Abc_ObjIsPo(node)) {
            reachablePOs.push_back(GetId(node));
        }
    }
    return reachablePOs;
}
...
std::vector <ABC_NAMESPACE::Abc_Obj_t *> NetMan::GetFanins(ABC_NAMESPACE::Abc_Obj_t * pObj) const {
    vector <Abc_Obj_t *> nodes;
...
std::vector <ABC_NAMESPACE::Abc_Obj_t *> NetMan::GetFanouts(ABC_NAMESPACE::Abc_Obj_t * pObj) const {
    vector <Abc_Obj_t *> nodes;
...
int NetMan::NodeDeref_rec(ABC_NAMESPACE::Abc_Obj_t* pRoot, ABC_NAMESPACE::Abc_Obj_t* pNode, unordered_set<ABC_NAMESPACE::Abc_Obj_t*>& divSet) const {
    Abc_Obj_t * pFanin;
    int i, Counter = 1;
...
int NetMan::NodeRef_rec(ABC_NAMESPACE::Abc_Obj_t* pRoot, ABC_NAMESPACE::Abc_Obj_t * pNode, unordered_set<ABC_NAMESPACE::Abc_Obj_t*>& divSet) const {
    Abc_Obj_t * pFanin;
    int i, Counter = 1;
    if ( Abc_ObjIsCi(pNode) || divSet.count(pNode) || (pRoot != pNode && IsPoDriver(pNode)) )
        return 0;
...
ABC_NAMESPACE::Abc_Obj_t* NetMan::CreateNode(const AbcObjVect& pFanins, const std::string& sop) {
    auto pNewNode = ABC_NAMESPACE::Abc_NtkCreateNode(GetNet());
...
int NetMan::CreateNodeAIG(const IntVect& faninIds, const std::string & sop) {
    ...
    if (sop == "01 1\n10 1\n" || sop == "10 1\n01 1\n" || sop == "00 0\n11 0\n" || sop == "11 0\n00 0\n") { // xor
        ...
        bool isComplement = ABC_NAMESPACE::Abc_SopIsComplement(pSop);
        int nCube = ABC_NAMESPACE::Abc_SopGetCubeNum(pSop);
        int nVar = ABC_NAMESPACE::Abc_SopGetVarNum(pSop);
        assert(nVar == 3);
        if (nCube == 1) {
            auto pCube = pSop;
...
// std::vector <ll> NetMan::TempRepl(ABC_NAMESPACE::Abc_Obj_t * pTS, ABC_NAMESPACE::Abc_Obj_t * pSS) {
//     #ifdef DEBUG
//     assert(pTS != pSS);
//     assert(pTS->pNtk == pSS->pNtk);
//     assert(ABC_NAMESPACE::Abc_ObjFanoutNum(pTS));
//     #endif
//     // record fanouts
...
//     PrintVect(ret, "\n");
//     // transfer fanouts
//     ABC_NAMESPACE::Abc_ObjTransferFanout(pTS, pSS);
//     return ret;
// }
...
IntVect NetMan::TempRepl(ABC_NAMESPACE::Abc_Obj_t * pTS, ABC_NAMESPACE::Abc_Obj_t * pSS) {
    assert(pTS != pSS);
    assert(pTS->pNtk == pSS->pNtk);
    assert(ABC_NAMESPACE::Abc_ObjFanoutNum(pTS));
    // record fanouts
...
void NetMan::PatchFanin( Abc_Obj_t * pObj, ll iFanin, Abc_Obj_t * pFaninOld, Abc_Obj_t * pFaninNew ) {
    ...
    assert( ABC_NAMESPACE::Abc_ObjFanin(pObj, iFanin) == pFaninOld );
...
ABC_NAMESPACE::Abc_Obj_t * NetMan::CreateGate(vector <Abc_Obj_t *> && fanins, const std::string & gateName) {
    auto pLib = (Mio_Library_t *)Abc_FrameReadLibGen();
...
void NetMan::PropagateConst(ll startId, bool print) {
    assert(GetNetType() == NET_TYPE::SOP);
    // initialize
...
void NetMan::DumpCFile(std::string&& fileName) {
    ...
    ABC_NAMESPACE::Abc_Obj_t * pObj = nullptr;
    int i = 0;
    ...
    Abc_NtkForEachNode(pNtk, pObj, i) {
        ...
        if (Abc_NodeIsConst0(pObj)) {
            ...
            if (this->GetNetType() == NET_TYPE::SOP)
                ...
                assert(0);
            int nVars = ABC_NAMESPACE::Abc_SopGetVarNum(pSop);
            if (ABC_NAMESPACE::Abc_SopIsComplement(pSop))
                oss << "!(\n";
            else
                oss << "(\n";
            for (char * pCube = pSop; *pCube; pCube += nVars + 3) {
                ...
                for (int k = 0; pCube[k] != ' '; k++) {
                    ABC_NAMESPACE::Abc_Obj_t * pFanin = Abc_ObjFanin(pObj, k);
    ...
    Abc_NtkForEachPo(pNtk, pObj, i) {
        ABC_NAMESPACE::Abc_Obj_t * pDriver = Abc_ObjFanin0(pObj);

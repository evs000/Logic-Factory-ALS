...
#define MY_ABC_PRT(a,t)    (ABC_NAMESPACE::Abc_Print(1, "%s =", (a)), ABC_NAMESPACE::Abc_Print(1, "%9.6f sec\n", 1.0*((double)(t))/((double)CLOCKS_PER_SEC)))
#define MY_ABC_PRTP(a,t,T) (ABC_NAMESPACE::Abc_Print(1, "%s =", (a)), ABC_NAMESPACE::Abc_Print(1, "%9.6f sec (%6.6f %%)\n", 1.0*((double)(t))/((double)CLOCKS_PER_SEC), ((double)(T))? 100.0*((double)(t))/((double)(T)) : 0.0))

namespace ABC_NAMESPACE{
    using namespace vacsem;
...
using std::endl;
using std::stack;
using ABC_NAMESPACE::Vec_Int_t;
using ABC_NAMESPACE::Vec_Str_t;
using ABC_NAMESPACE::Gia_Man_t;
using ABC_NAMESPACE::Gia_Obj_t;
using ABC_NAMESPACE::abctime;
using ABC_NAMESPACE::Tas_Man_t;
using ABC_NAMESPACE::Tas_Que_t;
using ABC_NAMESPACE::Tas_Cls_t;
...
static inline int   Tas_VarToLit( Tas_Man_t * p, Gia_Obj_t * pObj ) { assert( Tas_VarIsAssigned(pObj) ); return ABC_NAMESPACE::Abc_Var2Lit( Gia_ObjId(p->pAig, pObj), !Tas_VarValue(pObj) );     }
static inline int   Tas_LitIsTrue( Gia_Obj_t * pObj, int Lit ) { assert( Tas_VarIsAssigned(pObj) ); return Tas_VarValue(pObj) != ABC_NAMESPACE::Abc_LitIsCompl(Lit);                             }
...
//            Vec_IntPush( vCex, Abc_Var2Lit(Gia_ObjId(p->pAig,pVar), !Tas_VarValue(pVar)) );
            Vec_IntPush( vCex, ABC_NAMESPACE::Abc_Var2Lit(Gia_ObjCioId(pVar), !Tas_VarValue(pVar)) );
/*
...
{
    assert( ABC_NAMESPACE::Abc_Lit2Var(Lit) < Gia_ManObjNum(p->pAig) );
    assert( pClause->nLits >= 2 );
    assert( pClause->pLits[0] == Lit || pClause->pLits[1] == Lit );
    if ( pClause->pLits[0] == Lit )
        pClause->iNext[0] = p->pWatches[ABC_NAMESPACE::Abc_LitNot(Lit)];  
    else
        pClause->iNext[1] = p->pWatches[ABC_NAMESPACE::Abc_LitNot(Lit)];  
    if ( p->pWatches[ABC_NAMESPACE::Abc_LitNot(Lit)] == 0 )
        Vec_IntPush( p->vWatchLits, ABC_NAMESPACE::Abc_LitNot(Lit) );
    p->pWatches[ABC_NAMESPACE::Abc_LitNot(Lit)] = Tas_ClsHandle( p, pClause );
}
...
static inline Tas_Cls_t * Tas_ManCreateCls( Tas_Man_t * p, int hClause )
{
    ...
    for ( i = hClause; (pObj = pQue->pData[i]); i++ )
    {
        ...
        pClause->pLits[i-hClause] = ABC_NAMESPACE::Abc_LitNot( Tas_VarToLit(p, pObj) );
    }
...
static inline int Tas_VarFaninFanoutMax( Tas_Man_t * p, Gia_Obj_t * pObj )
{
    ...
    return ABC_NAMESPACE::Abc_MaxInt( Count0, Count1 );
}
...
//        p->pActivity[Id] += p->Pars.VarInc;
        ...
        if ( Tas_VarHasReasonCls( p, pObj ) )
        {
            ...
            pReason = Gia_ManObj( p->pAig, ABC_NAMESPACE::Abc_Lit2Var(pCls->pLits[0]) );
            assert( pReason == pObj );
            for ( j = 1; j < pCls->nLits; j++ )
            {
                pReason = Gia_ManObj( p->pAig, ABC_NAMESPACE::Abc_Lit2Var(pCls->pLits[j]) );
...
ll GetUnssignedCI(Tas_Man_t * p) {
    ...
    for (int i = 0; i < p->pAig->nObjs; i++) {
        ...
        if (ABC_NAMESPACE::Gia_ObjIsCi(pVar)) {
...
int My_Tas_ManSolve_rec( Tas_Man_t * p, int Level ) {
    ...
    if ( (hClause = ABC_NAMESPACE::Tas_ManPropagate( p, Level )) )
    {
    ...
    p->Pars.nJustThis = ABC_NAMESPACE::Abc_MaxInt( p->Pars.nJustThis, p->pJust.iTail - p->pJust.iHead );
...
int My_Tas_ManSolve_loop( Tas_Man_t * p ) {
    ...
    while (!_stack.empty()) {
        ...
        if (d.status == 0) { // before "propagate assignments"
            assert( !Tas_QueIsEmpty(&p->pProp) );
            if ( (d.hClause = ABC_NAMESPACE::Tas_ManPropagate( p, d.Level )) )
            {
...
int My_Tas_ManSolve( Tas_Man_t * p, Gia_Obj_t * pObj, Gia_Obj_t * pObj2 ) {
    ...
    p->Pars.nJustTotal = ABC_NAMESPACE::Abc_MaxInt( p->Pars.nJustTotal, p->Pars.nJustThis );
    if ( Tas_ManCheckLimits( p ) )
        RetValue = -1;
    return RetValue;
}
...
//    Gia_Man_t * pAigCopy = Gia_ManDup( pAig ), * pAigTemp;
    ...
    int i, status;
    abctime clk, clkTotal = ABC_NAMESPACE::Abc_Clock();
...
//    Gia_ManCollectTest( pAig );
    ...
    vStatus   = ABC_NAMESPACE::Vec_StrAlloc( Gia_ManPoNum(pAig) );
    vCexStore = ABC_NAMESPACE::Vec_IntAlloc( 10000 );
    vVisit    = ABC_NAMESPACE::Vec_IntAlloc( 100 );
    vCex      = ABC_NAMESPACE::Tas_ReadModel( p );
...
//            printf( "Unsolved %d.\n", i );
            ...
            p->timeSatUndec += ABC_NAMESPACE::Abc_Clock() - clk;
            continue;
        }
...
//        pRootCopy->fCompl0 = 0;
        ...
        if ( status == 1 )
        {
            ...
            p->timeSatUnsat += ABC_NAMESPACE::Abc_Clock() - clk;
            continue;
        }
        p->nSatSat++;
...
//        Gia_SatVerifyPattern( pAig, pRoot, vCex, vVisit );
        ...
        p->timeSatSat += ABC_NAMESPACE::Abc_Clock() - clk;
...
//    Gia_ManStop( pAigCopy );
    ...
    p->timeTotal = ABC_NAMESPACE::Abc_Clock() - clkTotal;
    if ( fVerbose )
...
void CircSatTest(cmdline::parser& opt) {
    ...
    // auto pPo = ABC_NAMESPACE::Abc_NtkPo(pNtk, 0);
    // auto pDriv = ABC_NAMESPACE::Abc_ObjFanin0(pPo);
    // ABC_NAMESPACE::Abc_NtkDeleteObj(pPo);
    // auto pInv = ABC_NAMESPACE::Abc_NtkCreateNodeInv(pNtk, pDriv);
    // auto pNewPo = ABC_NAMESPACE::Abc_NtkCreatePo(pNtk);
    // ABC_NAMESPACE::Abc_ObjAddFanin(pNewPo, pInv);
    abc.Comm("&get");

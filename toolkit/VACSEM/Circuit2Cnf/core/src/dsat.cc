...
using ABC_NAMESPACE::ABC_INT64_T;
using ABC_NAMESPACE::Aig_Man_t;
using ABC_NAMESPACE::sat_solver;
using ABC_NAMESPACE::Cnf_Dat_t;
using ABC_NAMESPACE::abctime;
using ABC_NAMESPACE::Vec_Int_t;
using ABC_NAMESPACE::l_True;
using ABC_NAMESPACE::l_False;
using ABC_NAMESPACE::l_Undef;
using ABC_NAMESPACE::lit;
using ABC_NAMESPACE::lbool;
...
//        bound = 0;
    ...
    for (c = s->qtail-1; c >= bound; c--) {
        int     x  = ABC_NAMESPACE::lit_var(s->trail[c]);
        var_set_value(s, x, varX);
        s->reasons[x] = 0;
        if ( c < lastLev )
            var_set_polar( s, x, !ABC_NAMESPACE::lit_sign(s->trail[c]) );
    }
    //printf( "\n" );

    for (c = s->qhead-1; c >= bound; c--)
        order_unassigned(s,ABC_NAMESPACE::lit_var(s->trail[c]));
...
int my_sat_solver_solve(sat_solver* s, lit* begin, lit* end, ABC_INT64_T nConfLimit, ABC_INT64_T nInsLimit, ABC_INT64_T nConfLimitGlobal, ABC_INT64_T nInsLimitGlobal) {
    ...
    ABC_NAMESPACE::sat_solver_set_resource_limits( s, nConfLimit, nInsLimit, nConfLimitGlobal, nInsLimitGlobal );

    // Perform assumptions:
    s->root_level = 0;
    for ( i = begin; i < end; i++ ) {
        if ( !ABC_NAMESPACE::sat_solver_push(s, *i) ) {
    ...
    status = ABC_NAMESPACE::sat_solver_solve_internal(s);
...
int My_Fra_FraigSat( Aig_Man_t * pMan, ABC_INT64_T nConfLimit, ABC_INT64_T nInsLimit, int nLearnedStart, int nLearnedDelta, int nLearnedPerce, int fFlipBits, int fAndOuts, int fNewSolver, int fVerbose ) {
    ...
    // derive CNF
    pCnf = ABC_NAMESPACE::Cnf_Derive( pMan, Aig_ManCoNum(pMan) );

...
using std::cout;
using std::endl;
using ABC_NAMESPACE::Aig_Obj_t;
using ABC_NAMESPACE::Abc_Ntk_t;
using ABC_NAMESPACE::Aig_Man_t;
using ABC_NAMESPACE::Cnf_Dat_t;
using ABC_NAMESPACE::Cnf_Man_t;
using ABC_NAMESPACE::Cnf_Cut_t;
using ABC_NAMESPACE::Vec_Ptr_t;
using ABC_NAMESPACE::Vec_Int_t;
using ABC_NAMESPACE::Aig_MmFixed_t;
...
Cnf_Man_t * My_Cnf_ManStart()
{
    ...
    ABC_NAMESPACE::Cnf_ReadMsops( &p->pSopSizes, &p->pSops );
    // allocate memory manager for cuts
    p->pMemCuts = ABC_NAMESPACE::Aig_MmFlexStart();
    p->nMergeLimit = 10;
    // allocate temporary truth tables
    p->pTruths[0] = ABC_ALLOC( unsigned, 4 * ABC_NAMESPACE::Abc_TruthWordNum(p->nMergeLimit) );
    for ( i = 1; i < 4; i++ )
        p->pTruths[i] = p->pTruths[i-1] + ABC_NAMESPACE::Abc_TruthWordNum(p->nMergeLimit);
    p->vMemory = ABC_NAMESPACE::Vec_IntAlloc( 1 << 18 );
    return p;
}
...
Cnf_Dat_t * My_Cnf_ManWriteCnf( Cnf_Man_t * p, Vec_Ptr_t * vMapped, int nOutputs )
{
    ...
    Vec_PtrForEachEntry( Aig_Obj_t *, vMapped, pObj, i )
    {
        ...
        if ( pCut->nFanins < 5 )
        {
            ...
            nLiterals += ABC_NAMESPACE::Cnf_SopCountLiterals( p->pSops[uTruth], p->pSopSizes[uTruth] ) + p->pSopSizes[uTruth];
            assert( p->pSopSizes[uTruth] >= 0 );
            nClauses += p->pSopSizes[uTruth];
        }
        else
        {
            nLiterals += ABC_NAMESPACE::Cnf_IsopCountLiterals( pCut->vIsop[1], pCut->nFanins ) + ABC_NAMESPACE::Vec_IntSize(pCut->vIsop[1]);
            nClauses += ABC_NAMESPACE::Vec_IntSize(pCut->vIsop[1]);
        }
        ...
        {
            nLiterals += ABC_NAMESPACE::Cnf_IsopCountLiterals( pCut->vIsop[0], pCut->nFanins ) + ABC_NAMESPACE::Vec_IntSize(pCut->vIsop[0]);
            nClauses += ABC_NAMESPACE::Vec_IntSize(pCut->vIsop[0]);
        }
...
//    memset( pCnf->pVarNums, 0xff, sizeof(int) * Aig_ManObjNumMax(p->pManAig) );
    ...
    // assign the clauses
    vSopTemp = ABC_NAMESPACE::Vec_IntAlloc( 1 << 16 );
    ...
    Vec_PtrForEachEntry( Aig_Obj_t *, vMapped, pObj, i )
    {
        ...
        Vec_IntForEachEntry( vCover, Cube, k )
        {
            *pClas++ = pLits;
            *pLits++ = 2 * OutVar; 
            pLits += ABC_NAMESPACE::Cnf_IsopWriteCube( Cube, pCut->nFanins, pVars, pLits );
        }
...
p->timeCuts = ABC_NAMESPACE::Abc_Clock() - clk;

    // find the mapping
clk = ABC_NAMESPACE::Abc_Clock();
    Cnf_DeriveMapping( p );
p->timeMap = ABC_NAMESPACE::Abc_Clock() - clk;
...
p->timeSave = ABC_NAMESPACE::Abc_Clock() - clk;
...
void My_NtkDarToCnf(ABC_NAMESPACE::Abc_Ntk_t * pNtk, char * pFileName, int fFastAlgo, int fChangePol, int fVerbose)
{
    Aig_Man_t * pMan;
    Cnf_Dat_t * pCnf;
    abctime clk = ABC_NAMESPACE::Abc_Clock();
    ...
    {
        ABC_NAMESPACE::Abc_Print( 1, "Abc_NtkDarToCnf: AIG check has failed.\n" );
        Aig_ManStop( pMan );
        return;
    }
    // perform balance
    ...
    // print stats
    {
        ABC_NAMESPACE::Abc_Print( 1, "CNF stats: Vars = %6d. Clauses = %7d. Literals = %8d.   ", pCnf->nVars, pCnf->nClauses, pCnf->nLiterals );
        ABC_NAMESPACE::Abc_PrintTime( 1, "Time", ABC_NAMESPACE::Abc_Clock() - clk );
    }

    // write CNF into a file
    Cnf_DataWriteIntoFile( pCnf, pFileName, 0, NULL, NULL );
    Cnf_DataFree( pCnf );
    ABC_NAMESPACE::Cnf_ManFree();
    Aig_ManStop( pMan );
}
...
NetMan BuildMit(NetMan& accNet, NetMan& appNet, NetMan& mitNet) {
    ...
    Abc_AigForEachAnd(pAccNet, pObj, i) {
        pObj->pCopy = Abc_AigAnd((ABC_NAMESPACE::Abc_Aig_t *)pNet->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj));
    ...
    Abc_AigForEachAnd(pMitNet, pObj, i) {
        pObj->pCopy = ABC_NAMESPACE::Abc_AigAnd((ABC_NAMESPACE::Abc_Aig_t *)pNet->pManFunc, Abc_ObjChild0Copy(pObj), Abc_ObjChild1Copy(pObj));
...
void MiterToCnf(NetMan& mit, string outp, int iOutp) {
    ...
    for (int i = 0; i <= mit.GetIdMax(); ++i) {
        if (mit.IsNode(i)) {
            auto gate = string(ABC_NAMESPACE::Mio_GateReadName(static_cast <ABC_NAMESPACE::Mio_Gate_t *> (mit.GetObj(i)->pData)));
            if (gate == "inv1") {
                auto fi0 = mit.GetFanin(i, 0);
                node_s_inp_inv[i] = mit.GetId(fi0);
                if (mit.IsNode(fi0)) {
                    auto fiGate = string(ABC_NAMESPACE::Mio_GateReadName(static_cast <ABC_NAMESPACE::Mio_Gate_t *> (mit.GetFanin(i, 0)->pData)));

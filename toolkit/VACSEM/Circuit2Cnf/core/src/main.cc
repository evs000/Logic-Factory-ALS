...
namespace ABC_NAMESPACE {
    void Abc_NtkRemovePo( ABC_NAMESPACE::Abc_Ntk_t * pNtk, int iOutput, int fRemoveConst0 );
    void Abc_NtkDropOneOutput( ABC_NAMESPACE::Abc_Ntk_t * pNtk, int iOutput, int fSkipSweep, int fUseConst1 );
}
...
parser CommPars(int argc, char * argv[]) {
    ...
    opt.add<string>("exact", 'e', "path to exact circuit", false, "./toolkit/VACSEM/Circuit2Cnf/input/mult15/mult15.blif");
    opt.add<string>("appr", 'a', "path to approximate circuit", false, "./toolkit/VACSEM/Circuit2Cnf/input/mult15/1_mult15_err_1.52588e-05_size_1780_depth_39.blif");
    opt.add<string>("deviation", 'd', "path to deviation-function circuit, meaningless for ER, only used for MED", false, "./toolkit/VACSEM/Circuit2Cnf/input/deviation-function/width_30_absolute_error.blif");
...
void Enumerate(parser& opt) {
    AbcMan abc;
    abc.Comm("read ./toolkit/VACSEM/Circuit2Cnf/input/standard-cell/mcnc_simple.genlib");
...
void ConvertForER(parser& opt) {
    ...
    // }
    // technology mapping
    abc.Comm("r ./toolkit/VACSEM/Circuit2Cnf/input/standard-cell/mcnc_simple.genlib");
...
// extern "C" void Abc_NtkRemovePo( ABC_NAMESPACE::Abc_Ntk_t * pNtk, int iOutput, int fRemoveConst0 );
// extern "C" void Abc_NtkDropOneOutput( ABC_NAMESPACE::Abc_Ntk_t * pNtk, int iOutput, int fSkipSweep, int fUseConst1 );
void ConvertForMED(parser& opt) {
    ...
    // split POs
    std::vector<ABC_NAMESPACE::Abc_Obj_t*> delPos;
    ...
    for (int i = 0; i < G.GetPoNum(); ++i) { // only keep the i-th PO
        ...
        for (int j = 0; j < GTemp.GetPoNum(); ++j) {
            if (i != j) {
                ABC_NAMESPACE::Abc_NtkDropOneOutput( GTemp.GetNet(), j, false, false );
...
int c2c_main(int argc, char* argv[]) {
    GlobStartAbc();
